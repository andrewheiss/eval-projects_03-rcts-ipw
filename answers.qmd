...
```{r}
#| label: fig-program-dag
#| fig-cap: Rain barrel program DAG
#| fig-alt:
#|   Directed acyclic graph (DAG) showing the observational causal structure
#|   of the rain barrel program. Rain barrel (blue, exposure) and water bill
#|   (orange, outcome) are positioned left and right in the center. Four
#|   confounders—yard size, home garden, environmental attitudes, and average
#|   temperature—are positioned vertically between them. Arrows from all four
#|   confounders point to both rain barrel and water bill, opening backdoor
#|   paths that prevent a simple comparison from recovering the causal effect.
#|   Home garden is also caused by yard size and environmental attitudes.
#| out-width: 75%
#| fig-align: center
#| echo: false

knitr::include_graphics("images/barrel-dag-observational.png")
```
...
```{r}
#| label: fig-rct-dag
#| fig-cap: Rain barrel program DAG as an RCT
#| fig-alt: |
#|   Directed acyclic graph (DAG) showing the causal structure under a
#|   randomized controlled trial. The layout is identical to the observational
#|   DAG, but the arrows from yard size, home garden, environmental attitudes,
#|   and average temperature into rain barrel are absent. Randomization severs
#|   these links, closing all backdoor paths and allowing the rain barrel →
#|   water bill arrow to be interpreted as a causal effect.
#| out-width: 75%
#| fig-align: center
#| echo: false

knitr::include_graphics("images/barrel-dag-rct.png")
```
...
#| fig-alt: |
#|   Point-range chart showing the proportion of households assigned to each
#|   group along the y-axis (displayed as percentages) and group (No barrel,
#|   Barrel) along the x-axis. An orange horizontal reference line marks 50%.
#|   The control group (No barrel) is assigned at roughly 45% and the
#|   treatment group (Barrel) at roughly 55%, with confidence intervals that
#|   do not overlap 50%, indicating a statistically significant imbalance in
...
#| fig-alt: |
#|   Four point-range plots arranged in a 2×2 grid. Each plot compares the
#|   mean value of a pre-treatment covariate (proportion with a home garden,
#|   yard size in square feet, attitude toward environment, temperature) for
#|   the No barrel and Barrel groups. In all four panels, the confidence
#|   intervals overlap substantially, indicating no statistically significant
#|   differences in pre-treatment characteristics across treatment and control
#|   groups.
#| fig-width: 8
#| fig-height: 6
#| out-width: 100%
